...
export default function AdvisorProfile() {
  ...
  useEffect(() => {
    if (bankEditMode) {
      // Ensure user can start typing immediately after clicking Edit.
  ...
  useEffect(() => {
    const loadBank = async () => {
      try {
        ...
        // Bank info is stored in LecturerProfile; if missing, keep form empty.
        console.warn("Failed to load bank info", e);
      } finally {
  ...
  const submitBank = async (e) => {
    ...
    try {
      setBankSaving(true);
      const res = await updateMyLecturerProfile({
  ...
  const handleBankPrimaryClick = async (e) => {
    // Defensive: prevent any accidental form submit/navigation.
    e.preventDefault();
    if (!bankEditMode) {
                ...
                {[
                  ...
                  <div key={field}>
                    ...
                    <div className="relative">
                      ...
                      >
                        {showPw[key] ? <Eye className="w-5 h-5" /> : <EyeOff className="w-5 h-5" />}
                      </button>
                    </div>
                  </div>
                ))}
            ...
            <h2 className="text-xl font-semibold text-slate-900 flex items-center">
              ...
              {import.meta?.env?.DEV ? (
                <span className="ml-3 text-xs font-medium text-slate-400">
                  mode: {bankEditMode ? "edit" : "view"}
                </span>
              ) : null}
            </h2>
            <p className="text-sm text-slate-600 mt-1">Manage your bank details for payments</p>
          </div>
          <div className="p-6">
            <form onSubmit={submitBank} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    disabled={bankSaving}
                    placeholder={bankLoading ? "Loading..." : "e.g. ABA Bank"}
                ...
                <div>
                  ...
                  <Input
                    ...
                    readOnly={!bankEditMode}
                    disabled={bankSaving}
                    placeholder={bankLoading ? "Loading..." : "Account holder name"}
                    className="border-slate-200 focus:border-indigo-500 focus:ring-indigo-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    readOnly={!bankEditMode}
                    disabled={bankSaving}
                    placeholder={bankLoading ? "Loading..." : "Account number"}
                    className="border-slate-200 focus:border-indigo-500 focus:ring-indigo-500"
                  />
                </div>
              </div>
              ...
              <div className="pt-2">
                <Button
                  ...
                  onClick={handleBankPrimaryClick}
                >
                  {bankSaving
                    ? "Saving..."
                    : bankEditMode
